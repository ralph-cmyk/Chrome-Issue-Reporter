...
const updateBaseUrl =
  options["update-url"] || process.env.UPDATE_BASE_URL || "";

if (!extensionId) {
  console.error(
    "Error: Extension ID required. Set EXTENSION_ID environment variable or use --extension-id=ID",
  );
  process.exit(1);
}
...
if (!updateBaseUrl) {
  console.error(
    "Error: Update base URL required. Set UPDATE_BASE_URL environment variable or use --update-url=URL",
  );
  process.exit(1);
}
...
try {
  execSync(
    `node scripts/update-manifest.js "${extensionId}" "${newVersion}" "${downloadUrl}" "${updateXmlPath}"`,
    {
      stdio: "inherit",
      cwd: path.join(__dirname, ".."),
    },
...
console.log(`\nNext steps:`);
console.log(
  `1. Upload ${zipFilename} to Cloudflare Workers/Pages at: ${updateBaseUrl}/extensions/`,
);
console.log(
  `2. Upload update.xml to Cloudflare Workers/Pages at: ${updateBaseUrl}/update.xml`,
);

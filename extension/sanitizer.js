...
const NON_DETERMINISTIC_ATTRS = ["nonce", "integrity", "crossorigin"];
...
function buildSanitizedIssue(context = {}, userInput = {}) {
  ...
  const title = sanitizeTitle(
    userInput.title || context.title || "Issue Report",
  );
...
function buildElementContext(context = {}) {
  ...
  if (context.selectedText) {
    rows.push(
      `- **Selected Text:** ${truncateAndRedact(context.selectedText, 180)}`,
    );
  }
...
function buildJsError(jsError) {
  ...
  if (jsError.source) {
    parts.push(
      `**Source:** ${redactText(jsError.source)}:${jsError.line || 0}:${jsError.column || 0}`,
    );
  }
...
function buildConsoleSummary(logs) {
  ...
  const latestError = logs
    .slice()
    .reverse()
    .find((log) => log.type === "error");
  const latestWarn = logs
    .slice()
    .reverse()
    .find((log) => log.type === "warn");

  const lines = [];
  lines.push(
    `- Errors: **${errors}** • Warnings: **${warns}** • Infos: ${infos} • Debug: ${debugs}`,
  );
